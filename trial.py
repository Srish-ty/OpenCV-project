...
for col in range(7):
    for r in range(7):
        if r!=6:
            if all(maze_img[ 100*col+5, 100*r+60 ] == [255,255,255]):
                verti_roads.append( horiz[col]+ str(r+1)+'-'+ horiz[col]+str(r+2) )
                maze_img= cv2.circle(maze_img,( 100*r+60, 100*col+5),10,(200,0,255),4)
        if col!=6:
            if all(maze_img[100*col+60, 100*r+5] == [255,255,255]):
                hori_roads.append( horiz[col]+str(r+1)+'-'+ horiz[col+1]+str(r+1) )
                maze_img= cv2.circle(maze_img,( 100*r+5, 100*col+60),10,(200,200,0),4)
